...
%Class responsible for decoding chirps - contains all functons pertaining to decoding
    ...
    methods
        function self = my_Decoder(Y,r,l,parity,re,m,p,K)
            addpath('utils');
            self.Y=Y;
            self.r=r;
            self.parity=parity;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [output_bits] = my_chirrup_decode(self)
...
%                             sparsity_ratio = 3;
                            %ceil():ceiling function 上の整数の一番近い数にする e.g. ceil(3.4)→4
                            %recov = chirp_rec(Yの1つずつのslot,『param(今は3) × K/2^(p-1)『（天井関数）, slot番号)
%                             recov = self.chirp_rec(Yp(:,slot),ceil(self.params.sparsity_factor*self.K/2^(self.p-1)),slot);
                            recov = self.my_chirp_rec(Yp(:,slot),0.05,slot);
                            disp(recov);
                     
                            %もうここで2^p個のslotを回している
                            count = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function recov = my_chirp_rec(self,y,nitlim,slot)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function bits = find_bits(self,res)
            ...
            for k = 1:K
                ...
                ck = res(k).c;
                compsk = res(k).comps;
                [i j] = upper_indices(self.re, self.m);
                for l = 1:length(i)
                    Pvec(l) = Pk(i(l),j(l));
                end
                bits2 = [Pvec bk];
